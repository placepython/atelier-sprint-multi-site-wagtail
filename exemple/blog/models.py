...
class BlogPage(RoutablePageMixin, Page):
    ...
    def get_posts(self):
        return (
            PostPage.objects.descendant_of(self).live().order_by("-post_date")
        )
    ...
    def post_by_date_slug(
        self, request, year, month, day, slug, *args, **kwargs
    ):
    ...
    def post_by_category(self, request, category, *args, **kwargs):
        ...
        self.posts = self.get_posts().filter(
            categories__blog_category__slug=category
        )
        return self.render(request)
    ...
    @route(r"^(\d{4})/(\d{2})/(\d{2})/$")
    def post_by_date(
        self, request, year, month=None, day=None, *args, **kwargs
    ):
...
class PostPage(MetadataPageMixin, Page):
    ...
    def canonical_url(self):
        ...
        from wagtail_app.blog.templatetags.blogapp_tags import (
